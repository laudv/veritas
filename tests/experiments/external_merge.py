...
def prepare_xgb_model_json_multiclass(xgbjson, columns, label, target_label, num_classes):
    ...
    return prepare_xgb_model_json(xgbjson1, columns, negate_leaf=True) + prepare_xgb_model_json(
        xgbjson0, columns, negate_leaf=False
    )
...
def run_process(max_mem=8 * 1024 * 1024 * 1024, max_time=60 * 60 * 24):
    ...
    p = subprocess.Popen([MERGE_EXECUTABLE, CONFIG_FILE], stdout=subprocess.PIPE, preexec_fn=limit_virtual_memory)
    exception = False
    try:
...
def external_merge_binary(dataset, example_is, start_delta, max_clique, max_level):
...
def external_merge_multiclass(dataset, example_is, start_delta, max_clique, max_level, num_classes):
    xgbmodel = dataset.model
    columns = dataset.meta["columns"]
    examples = [list(dataset.X.iloc[example_i, :]) for example_i in example_is]
    example_labels = [int(dataset.y[example_i]) for example_i in example_is]
    ...
    def write_model(label, target_label):
        with open(MODEL_FILE, "w") as f:
            dump = xgbmodel.get_dump(dump_format="json")
            dump = prepare_xgb_model_json_multiclass(dump, columns, label, target_label, num_classes)
            json.dump(dump, f)
    ...
    # SORT deltas, times
    x = sorted(sorted(zip(permutation, deltas, times), key=lambda p: p[0][1]), key=lambda p: p[0][0])
