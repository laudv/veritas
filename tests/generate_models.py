...
def generate_img_regression():
    ...
    params = {
        "xgb-img-very-easy-new": {"max_depth": 3, "learning_rate": 1.0, "n_estimators": 3},
        "xgb-img-easy-new": {"max_depth": 6, "learning_rate": 0.5, "n_estimators": 10},
        "xgb-img-hard-new": {"max_depth": 6, "learning_rate": 0.25, "n_estimators": 40},
    }

    for k, ps in params.items():
        print("TRAIN", k)
        regr = xgb.XGBRegressor(objective="reg:squarederror", tree_method="hist", **ps)
        model = regr.fit(X, y)
...
def generate_img_multiclass():
    ...
    clfs = {
        "xgb-img-multiclass": xgb.XGBClassifier(
            objective="multi:softmax", num_class=4, tree_method="hist", max_depth=6, learning_rate=0.5, n_estimators=20
        ),
        ...
        "rf-img-multiclass": RandomForestClassifier(max_depth=8, random_state=0, n_estimators=25),
    }
