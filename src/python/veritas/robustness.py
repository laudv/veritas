...
from . import Config, HeuristicType, Interval, get_closest_example
...
class RobustnessSearch:
    ...
    def __init__(
        ...
        # prune addtree
        self.prune_box = [Interval(example[f] - start_delta, example[f] + start_delta) for f in range(len(example))]
    ...
    def search(self):
        ...
        for self.i in range(self.num_steps):
            ...
            if max_output_diff >= 0.0:
                ...
                if not self.silent:
                    t = timeit.default_timer() - self.start_time
                    print(f"[{self.i} {t:3.1f}s]: UNSAT for delta {old_delta:.5f} -> {delta:.5f} [{lower:.5f}, {upper:.5f}]")
...
class VeritasRobustnessSearch(RobustnessSearch):
    ...
    def get_search(self, delta):
        ...
        config.stop_when_num_solutions_exceeds = self.stop_when_num_solutions_exceeds
...
class MilpRobustnessSearch(RobustnessSearch):
    def __init__(self, example, start_delta, source_at, target_at, silent=True, **kwargs):
        super().__init__(example, start_delta, source_at, target_at, **kwargs)
        self.silent = silent
